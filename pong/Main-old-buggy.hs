...
movingBall input time = do
  return $ (drawScene <$> move' input) &>
           (stepper (return ())
...
-}
  ...
  lVel (GLFW.KeyLeft,True)   x = x .+. (-4,0)
  lVel (GLFW.KeyLeft,False)  x = x .+. (0,0)
  rVel (GLFW.KeyRight,True)  x = x .+. (4,0)
  rVel _                     x = x .+. (0,0)
  uVel (GLFW.KeyUp,True)     x = x .+. (0,4)
  uVel _                     x = x .+. (0,0)
  dVel (GLFW.KeyDown,True)   x = x .+. (0,-4)
  dVel _                     x = x .+. (0,0)

  rights :: Event t (GLFW.Key,Bool) -> Event t (GLFW.Key,Bool)
  rights i = filterE (\(k,b) -> k == GLFW.KeyRight) i
  lefts  i = filterE (\(k,b) -> k == GLFW.KeyLeft)  i
  ups    i = filterE (\(k,b) -> k == GLFW.KeyUp)    i
  downs  i = filterE (\(k,b) -> k == GLFW.KeyDown)  i

  (a,b) .+. (c,d) = (a+c,b+d)

  -- This is just: right k + left k + up k + down k
  pos k = (.+.) <$> ((.+.) <$> right' k <*> left k) <*> ((.+.) <$> up k <*> down k)

  right :: Event t (GLFW.Key,Bool) -> Behavior t (GLfloat,GLfloat)
  right k = accumB (0,0) (rVel <$> rights k)
  left  k = accumB (0,0) (lVel <$> lefts  k)
  up    k = accumB (0,0) (uVel <$> ups    k)
  down  k = accumB (0,0) (dVel <$> downs  k)


  isRight = accumB False ((\b _ -> snd b) <$> rights k)

  right' :: Event t (GLFW.Key,Bool) -> Behavior t (GLfloat,GLfloat)
  right' k = switchBehaviors (pure (4,0))
                             (pure (0,0))
                             (accumB False ((\b _ -> snd b) <$> rights k))
  ...
  switchEvents 
      :: a    -- | value to yield when it's "True"
      -> a    -- | value to yield when it's "False"
      -> Event t Bool -- | Select between values
      -> Event t a
  switchEvents t f es =
    apply (pure (\e -> if e then t else f)) es

  move' :: Event t (GLFW.Key,Bool) -> Behavior t Ball
  move' key = moveBall <$> pos key <*> pure zeroBall
  -- move' key = accumB zeroBall (pos key)
...
gameLoop gameNetwork = do
    ...
    network <- compile $ do
        ...
        bGraphics <- gameNetwork eKeyInput bTime
